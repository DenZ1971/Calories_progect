...
*===================================*/
var rescal=0;
	function rsachetKalor(){
		var A = $('#calculation #weight').val()!=''?$('#calculation #weight').val():-500,
			B = $('#calculation #height').val()!=''?$('#calculation #height').val():-500,
			C = $('#calculation #age').val()!=''?$('#calculation #age').val():-500,
			F = +$('#calculation [name="payment_option"]:checked').val(),
			H = $('#calculation [name="gender"]:checked').val() == 'female' ? 447.6 : 88.3;
		var result = (H+((H==447.6?9.2:13.4)*A)+((H==447.6?3.1:4.8)*B)-((H==447.6?4.3:5.7)*C))*F;
		result = Math.ceil(result * 100) / 100;
		var result2 = result*0.82;
		result2 = Math.ceil(result2 * 100) / 100;
		var result3 = result*1.15;
		result3 = Math.ceil(result3 * 100) / 100;
		// console.log(A,B,C,F,H,result);
		// если хочешь похудеть R2-(R2×18%)
		// если хочешь набрать массу (R2×15%)+R2
		if(String(parseFloat(result, 10)) === String(result) && result > 0){
			$('#calculation #resultcalor').html('Ваша калорийность: '+result+' ккал<br>Если необходимо похудеть: '+result2+' ккал<br>Для набора массы: '+result3+' ккал');
			// $({numberValue: rescal}).animate({numberValue: result}, {
			...
			$('#calculation #resultcalor').text('Ваша Калорийность: не рассчитана');
...
(function($) {
    ...
    $(document).on('click', '.btn-outline-default', function(event) {
    ...
    $(document).on('click', '.vopros .btn-success', function(event) {
		var params = $(this).closest('form').serialize();
		$.ajax({
			url: "/0ajax.php",
			type: "POST",
			data: {vopros: '1', param: params},
			success: function(data) {eval(data);}
		});
            event.preventDefault();
	});
		
		$('#calculation input').on('change keyup', function(){clearTimeout(timecal);timecal = setTimeout(rsachetKalor(),2000);});
